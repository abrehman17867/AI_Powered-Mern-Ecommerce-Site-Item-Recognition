...
const createProduct = async (req, res) => {
  try {
    ...
    const imageUrl =
      json.imageUrl || `${req.protocol}://${req.get("host")}/uploads/${json.photo}`;
    return res.status(201).send({ ...json, imageUrl });
  } catch (error) {
...
const createMultipleProduct = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .send({ message: "Products Created Successfully", count: products.length });
  } catch (error) {
    return res.status(500).send({ error: error.message });
  }
};
...
const uploadImage = async (req, res) => {
  try {
      ...
      const prediction = await predictImageLabel(imagePath);
      res.json({ prediction });
  } catch (error) {
